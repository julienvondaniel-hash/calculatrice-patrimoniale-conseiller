...
const Screens = {
  ...
  home() {
    const v = el('div', {});
    const h = hero('CALCULATRICE CONSEILLER', 'Hexa');
    const plaque = el('div', { style: 'background:#fff;border-radius:18px;padding:12px 18px;max-width:230px;margin:2px auto 14px;box-shadow:0 8px 24px rgba(0,0,0,.18)' }, [
      el('img', { src: 'logo-hexa.png', alt: 'HEXA Patrimoine', style: 'width:100%;height:auto;display:block' })
    ]);
    h.insertBefore(plaque, h.firstChild);
    v.append(h);
  ...
  account() {
    ...
    sheet.append(c);
    sheet.append(el('button', { class: 'btn-ghost', style: 'margin-top:8px', onclick: () => Auth.signOut() }, 'Se déconnecter'));
    v.append(sheet);
    return v;
  },
